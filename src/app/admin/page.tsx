...
const DatabaseSetupPanel = () => {
  ...
  const [v,setV]=useState({setupCompleted:false,primary:'upstash',upstashUrl:'',upstashToken:'',redisUrl:'',mysqlHost:'127.0.0.1',mysqlPort:3306,mysqlUser:'root',mysqlPassword:'',mysqlDatabase:'moontv',mysqlTablePrefix:'moontv_',syncEnabled:true,latencyEnabled:true,doubanDataUrls:[],doubanImageUrls:[],cdnUrls:[]});
  ...
  return <div className='mb-6 rounded-xl border-2 border-amber-400 bg-amber-50 p-5 dark:bg-amber-950/30'><h2 className='text-xl font-bold'>首次数据库与自动测速设置</h2><p className='my-2 text-sm'>此步骤只显示一次。保存时会测试 Redis、创建 MySQL 数据表，并写入后续自动同步配置。运行时优先 Upstash，失连则自动回退 Redis、MySQL。</p><div className='grid gap-3 md:grid-cols-2'>{field('upstashUrl','Upstash REST URL')}{field('upstashToken','Upstash Token','password')}{field('redisUrl','Redis URL')}{field('mysqlHost','MySQL 地址')}{field('mysqlPort','MySQL 端口','number')}{field('mysqlUser','MySQL 用户')}{field('mysqlPassword','MySQL 密码','password')}{field('mysqlDatabase','MySQL 数据库')}{field('mysqlTablePrefix','MySQL 数据表前缀')}</div><div className='mt-3 grid gap-3 md:grid-cols-3'><textarea className='rounded border p-2 dark:bg-gray-900' placeholder='豆瓣数据候选 URL，每行一个' value={dataUrls} onChange={e=>setDataUrls(e.target.value)}/><textarea className='rounded border p-2 dark:bg-gray-900' placeholder='豆瓣图片候选 URL，每行一个' value={imageUrls} onChange={e=>setImageUrls(e.target.value)}/><textarea className='rounded border p-2 dark:bg-gray-900' placeholder='CDN 候选 URL，每行一个' value={cdnUrls} onChange={e=>setCdnUrls(e.target.value)}/></div><button className='mt-4 rounded-lg bg-amber-600 px-4 py-2 text-white disabled:opacity-50' disabled={saving} onClick={save}>{saving?'正在测试并初始化…':'测试连接、建表并完成首次同步设置'}</button></div>;
};
